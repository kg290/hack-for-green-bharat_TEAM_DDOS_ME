...
| Metric | Score |
|--------|-------|
| **Test mIoU (7 present classes)** | **0.6501** |
| **Test mIoU (all 10 classes)** | **0.4551** |

> 3 out of 10 classes (Ground Clutter, Flowers, Logs) are entirely absent from the test set, making the 7-class mIoU the most representative evaluation metric.
...
| Epochs | 50 |
| Result | **Val mIoU (10-class) = 0.6338, (7-class) = 0.6913** (Epoch 45) |
...
|-------|:--------:|:-------------:|
| Trees | 0.6423 | 0.27% |
| Lush Bushes | 0.0967 | ~0% |
| Dry Grass | 0.7534 | 17.4% |
| Dry Bushes | 0.6845 | 3.05% |
| Ground Clutter | 0.0000 | 0% |
| Flowers | 0.0000 | 0% |
| Logs | 0.0000 | 0% |
| Rocks | 0.5267 | 18.1% |
| Landscape | 0.8630 | 43.2% |
| Sky | 0.9841 | 18.0% |
...
| **Test mIoU (7 present classes)** | **0.6501** |
| **Test mIoU (all 10 classes)** | **0.4551** |
| Test Pixel Accuracy | 0.8124 |
| Avg Inference Time | 224.2 ms/image (multi-scale TTA) |

> **Note:** The 10-class mIoU (0.4551) includes 3 classes entirely absent from the test set (Ground Clutter, Flowers, Logs), each contributing 0.0 to the average. The **7-class mIoU (0.6501)** is the more representative and fair measure of model performance on the test environment, and is our primary reported **IOU Score**.

<div align="center">
<img src="final_per_class_iou.png" alt="Final Per-Class IoU" width="70%"/>

*Per-class IoU on test set — Sky (0.98) and Landscape (0.86) perform well, with Rocks (0.53) showing significant improvement from domain-generalisation augmentation.*
</div>

<div align="center">
<img src="final_confusion_matrix.png" alt="Test Confusion Matrix" width="60%"/>

*Test confusion matrix — reveals improved Rock segmentation through copy-paste augmentation and weighted sampling.*
</div>
...
This domain shift causes the validation → test performance gap (7-class val mIoU 0.6913 → 7-class test mIoU 0.6501; 10-class val mIoU 0.6338 → 10-class test mIoU 0.4551). The v4 augmentation strategy with copy-paste, weighted sampling, and aggressive colour jitter was specifically designed to close this gap, achieving strong generalisation to the unseen test environment.
...
1. **Rocks (IoU: 0.5267):** Most improved class thanks to copy-paste augmentation and weighted sampling. Rocks in the test environment have drastically different texture/colour from training, but the domain-generalisation strategy significantly boosted performance.
2. **Trees (IoU: 0.6423):** Good performance despite domain shift. The strong colour jitter and grayscale augmentation helped the model generalise across different tree appearances.
3. **Lush Bushes (IoU: 0.0967):** Low — Lush Bushes are nearly absent in the test environment, limiting achievable IoU.
4. **3 Absent Classes (IoU: 0.0000):** Ground Clutter, Flowers, and Logs not present in test.

### Why the Gap Exists

The synthetic dataset from Duality AI's Falcon platform uses two distinct desert environments for train and test. While the model learns strong features on the training environment (10-class val mIoU 0.6338, 7-class val mIoU 0.6913), these features transfer well to the test environment (7-class test mIoU 0.6501, 10-class test mIoU 0.4551) thanks to our domain-generalisation strategy. The remaining gap is due to:
...
| Phase 2 v4 | Val mIoU (10-class) | ≈ 0.6338 (epoch ~45) |
| Phase 2 v4 | Test-Relevant Val mIoU | ≈ 0.6913 |
| Test | mIoU (7 present classes) | ≈ **0.6501** |
| Test | mIoU (10-class) | ≈ 0.4551 |
