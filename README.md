...
This domain shift causes the validation → test performance gap (val mIoU 0.6338 → test 7-class mIoU 0.6501). The v4 augmentation strategy with copy-paste, weighted sampling, and aggressive colour jitter was specifically designed to close this gap, achieving strong generalisation to the unseen test environment.
...
The synthetic dataset from Duality AI's Falcon platform uses two distinct desert environments for train and test. While the model learns strong features on the training environment (val mIoU 0.6338), these features transfer well to the test environment (7-class test mIoU 0.6501) thanks to our domain-generalisation strategy. The remaining gap is due to:
